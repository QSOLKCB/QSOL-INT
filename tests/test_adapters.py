...
class AdapterTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.by_kind = {Path(path).stem: fixture for path, fixture, _ in cls.fixtures}

    def test_fixture_matrix_covers_every_required_case(self) -> None:
        self.assertEqual(set(self.by_kind), {"valid", "invalid", "unknown", "conflict", "satire", "cross-mode", "missing-provenance", "drift"})
    ...
    def test_all_three_adapters_preserve_valid_semantics(self) -> None:
        ...
        for adapter in adapt.ADAPTERS:
            with self.subTest(adapter=adapter):
                ...
                self.assertEqual(output["semantic_receipt"]["annotations"], fixture["annotations"])

    def test_unknown_remains_unknown(self) -> None:
        fixture = self.by_kind["unknown"]
        for adapter in adapt.ADAPTERS:
            output = adapt.process(adapter, fixture)
            self.assertEqual(output["semantic_receipt"]["annotations"]["epistemic_state"], "unknown")
    ...
    def test_cross_mode_without_bridge_is_blocked(self) -> None:
        ...
        self.assertEqual(output["failure_code"], "INT_CROSS_MODE_BRIDGE_REQUIRED")

    def test_missing_provenance_is_rejected(self) -> None:
        output = adapt.process("generic", self.by_kind["missing-provenance"])
        self.assertEqual(output["decision"], "reject")
        self.assertEqual(output["failure_code"], "INT_ADAPTER_PROVENANCE_REQUIRED")
    ...
    def test_malformed_envelope_is_rejected_with_typed_code(self) -> None:
        ...
        self.assertEqual(output["decision"], "reject")
        self.assertEqual(output["failure_code"], "INT_ADAPTER_ENVELOPE_INVALID")

    def test_semantic_receipt_tampering_is_detected(self) -> None:
        fixture = self.by_kind["valid"]
        original = copy.deepcopy(fixture)
        output = adapt.process("generic", fixture)
        output["semantic_receipt"]["annotations"]["epistemic_state"] = "unknown"
        self.assertEqual(fixture, original)
        with self.assertRaisesRegex(adapt.AdapterFailure, "INT_ADAPTER_SEMANTIC_LOSS"):
            adapt.validate_output(output, fixture)
    ...
    def test_transport_payload_is_detached_from_source_fixture(self) -> None:
        ...
        original = copy.deepcopy(fixture)
        output = adapt.process("generic", fixture)
        output["transport"]["payload"]["messages"][0]["content"] = "tampered"
        self.assertEqual(fixture, original)
        with self.assertRaisesRegex(adapt.AdapterFailure, "INT_ADAPTER_SEMANTIC_LOSS"):
            adapt.validate_output(output, fixture)
    ...
    def test_committed_conformance_report_regenerates_exactly(self) -> None:
        report = adapt.load_json("adapters/reports/conformance.json", "INT_ADAPTER_REPORT_INVALID")
        adapt.validate_report(report)

    def test_unknown_adapter_is_rejected(self) -> None:
        output = adapt.process("telepathy", self.by_kind["valid"])
        self.assertEqual(output["failure_code"], "INT_ADAPTER_UNKNOWN")
